...
function definedOptions(options: GridOptions): Partial<Required<GridOptions>> {
  return Object.fromEntries(
    Object.entries(options).filter(([, value]) => value !== undefined),
  ) as Partial<Required<GridOptions>>;
}
...
export function createGrid(elements: GridElements, options: GridOptions = {}): GridInstance | null {
  const config = {
    ...DEFAULTS,
  ...
  function frame(now: number) {
    ...
    const settling = Math.abs(vanishX - vanishTargetX) + Math.abs(vanishY - vanishTargetY) > 0.001;
  ...
  return {
    setOptions(next) {
      Object.assign(config, definedOptions(next));
      start();
    },
    resize() {
      syncCanvasSize();
